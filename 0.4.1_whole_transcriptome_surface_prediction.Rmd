...
# Extract the RAW counts and apply log-CPM normalisation
...
# Apply Log-CPM normalization (same as script 3)
gene_cols <- colnames(expr_df)
total_counts <- rowSums(expr_df[, gene_cols])
scale_factor <- 1e6

expr_df[, gene_cols] <- log1p(
  sweep(expr_df[, gene_cols], 1, total_counts, "/") * scale_factor
)
...
ggplot(results_df, aes(x = LD1, fill = predicted_class)) +
  ...
  geom_vline(xintercept = c(-1.0, 0), linetype = "dashed", color = "black") +
...
wts_obj_invasive$predicted_surface_class <- results_df$predicted_class[
  match(colnames(wts_obj_invasive), results_df$cell_id)
]
wts_obj_invasive$prob_core <- results_df$prob_core[
  match(colnames(wts_obj_invasive), results_df$cell_id)
]
wts_obj_invasive$LD1_score <- results_df$LD1[
  match(colnames(wts_obj_invasive), results_df$cell_id)
]
