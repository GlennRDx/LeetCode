...
n = 3 # number of pseudo-replicates
```
...
cell_assignments <- sample(1:n, n_cells, replace = TRUE)

surface_pseudobulk_WTD <- sapply(1:n, function(i) {
  cells_in_sample <- which(cell_assignments == i)
  rowSums(scRNA_surface_expr[, cells_in_sample, drop = FALSE])
})

# Set column names
colnames(surface_pseudobulk_WTD) <- paste0("surface_sample", 1:n)
```
...
core_pseudobulk_WTD <- sapply(1:n, function(i) {
  cells_in_sample <- which(cell_assignments == i)
  rowSums(scRNA_core_expr[, cells_in_sample, drop = FALSE])
})

# Set column names
colnames(core_pseudobulk_WTD) <- paste0("core_sample", 1:n)
```

### Merge the two raw count matrices together in preparation for DGE analysis.  
```{r}
# merge two expression matrices together by gene name (stored in rownames)
counts_WTD <- cbind(core_pseudobulk_WTD, surface_pseudobulk_WTD)

# filter to only the intersecting genes (between WTD, and ISS data)
shared_genes <- readRDS('janesick_2023/processed/shared_genes.RDS')
counts_WTD <- counts_WTD[rownames(counts_WTD) %in% shared_genes,]

# meta data for DESeq2
coldata <- data.frame(
...
```{r}
keep <- rowSums(counts(dds_WTD)) >= 10
dds_WTD <- dds_WTD[keep, ]
```
...
surface_pseudobulk_ISS <- sapply(1:n, function(i) {
  cells_in_sample <- which(cell_assignments == i)
  rowSums(ISS_surface_expr[, cells_in_sample, drop = FALSE])
})

# Set column names
colnames(surface_pseudobulk_ISS) <- paste0("surface_sample", 1:n)
```
...
core_pseudobulk_ISS <- sapply(1:n, function(i) {
  cells_in_sample <- which(cell_assignments == i)
  rowSums(ISS_core_expr[, cells_in_sample, drop = FALSE])
})

# Set column names
colnames(core_pseudobulk_ISS) <- paste0("core_sample", 1:n)
```

### Merge the two raw count matrices together in preparation for DGE analysis.  
```{r}
# merge two expression matrices together by gene name (stored in rownames)
counts_ISS <- cbind(core_pseudobulk_ISS, surface_pseudobulk_ISS)
...
### Filter low-count genes.  
```{r}
keep <- rowSums(counts(dds_ISS)) >= 10
dds_ISS <- dds_ISS[keep, ]
```
...
load("train_genes.RData")
comparison_full$training_gene <- comparison_full$gene %in% train_genes

comparison <- comparison_full[comparison_full$training_gene == TRUE,]
```

### Correlation Analysis
```{r}
cor_pearson <- cor(comparison$log2FoldChange_ISS, 
                   comparison$log2FoldChange_WTD, 
                   use = "complete.obs")
cor_spearman <- cor(comparison$log2FoldChange_ISS, 
                    comparison$log2FoldChange_WTD, 
                    method = "spearman", 
                    use = "complete.obs")
...
### Concordance Table
```{r}
table(comparison$sig_ISS, comparison$sig_WTD)
```

### Four Quadrant Scatter Plot
```{r}
ggplot(comparison, aes(x = log2FoldChange_ISS, y = log2FoldChange_WTD)) +
...
```{r}
comparison <- comparison_full[comparison_full$training_gene == FALSE,]
```
